...
<div class="block__cache">
    ...
    ?>
        <div class="b_cache--item">
            <a class="b_cache--link" href="<?= $schemaDto->getUpdateHref(); ?>">
                <svg xmlns="http://www.w3.org/2000/svg" fill="#000" width="28px" height="28px" viewBox="0 0 32 32">
            ...
            <label class="b_cache--label">
                <input class="b_cache--checkbox" type="checkbox" value="generate"
                       name="Crafter[generateList][<?= $schemaDto->getTableName() ?>]"
                       title="Generate files for <?= $schemaDto->getTableName() ?>"
                <?= ( $schemaDto->isPreviewGenerate($generator->generateList) ) ? 'checked' : '' ?>>

                <?= $schemaDto->displayTablename() ?>
            </label>
            <button class="b_cache--button" onclick="app.cache.removeModel('<?=$schemaDto->getTableName()?>')" type="button">
